...
# Universal Audit Skill

## Step 0: Load Procedure

Read `${CLAUDE_PLUGIN_ROOT}/UNIVERSAL-AUDIT.md` before doing anything else. This file is the single source of truth for all scoring rules, DSE definitions, fallback chain, and the 12-step procedura.
...
**What it does:** Single-pass Opus audit. No cross-model, no confidence scorer, no Ralph loop.
...
11. Verdict
12. **SKIP** Ralph loop (report and stop)

**Output:** Compact markdown — NPLF table + findings + verdict. No HTML, no Cortex log (unless `--cortex` also passed).

**When to use:** Quick quality check, mid-session review, pre-commit sanity check. When you don't need the full cross-model triangle.

---

## Steps 3–12: Follow UNIVERSAL-AUDIT.md §2 (Full Mode)

Do not duplicate the procedure here — execute §2 Steps 3–12 from `UNIVERSAL-AUDIT.md` directly.
...
When `model-availability.sh` returns **CODEX_OPUS** or **FULL_TRIANGLE**, dispatch GPT-5.4 audit in parallel with Opus agents:
...
The script handles: auth check, sanitize, `codex exec --model gpt-5.4 --sandbox read-only --output-schema`, exponential backoff, JSON validation. Returns path to findings JSON file.

**Merge GPT-5.4 findings with Opus findings at Step 8:**
1. Read JSON output from dispatch script
2. Each finding gets `source: "gpt-5.4"` (enforced by schema)
...
**If dispatch fails:** Log warning, continue with Opus-solo. Never block audit for an external model failure.
...
Full formula in `UNIVERSAL-AUDIT.md` §Hybrid Scoring. Summary:
...
State file: `~/.claude/universal-audit-loop.local.md` (runtime-only, auto-created, auto-deleted on completion, not committed).
...
3. Cortex log entry (always) — see §3 in `UNIVERSAL-AUDIT.md`
